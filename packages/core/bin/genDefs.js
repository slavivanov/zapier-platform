...
const recurse = (item, replacerFunc) => {
  if (isPlainObject(item)) {
    // if an object has a description and a reference, we need to wrap it to preserve the target class comments
    if (item.$ref) {
      const { $ref, ...otherProps } = item;
      const transformedRef = transformReference($ref);
      if (otherProps.description && otherProps.oneOf) {
        console.log(
          'WARNING! item with description',
          otherProps.description,
          'just got its oneOf clobbered'
        );
      }
      const refObj = { $ref: transformedRef };
      return {
        ...recurse(otherProps),
        // only need to wrap the ref if there's a description
        ...(otherProps.description
          ? {
              oneOf: [refObj],
            }
...
const formatTitleFromId = (s) => s.slice(1, -6);
...
const recursed = Object.entries(exportedSchema.schemas).reduce(
  (output, [schemaName, schema]) => {
    ...
    return {
      ...output,
      [schemaName]: {
        // put this first so the schema can overwrite it
        title: formatTitleFromId(id),
        ...recurse(restOfSchema),
        $id: transformId(id),
      },
    };
  },
